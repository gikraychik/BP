<?php 
	...
	// Get site content with ignore errors
	@$page_content->loadHTML('<?xml encoding="UTF-8">'.Site::Content());

	// Get fix price per hour
	$fix_price = $page_content->getElementsByTagName("fix-price")->item(0)->textContent;

	// Get sales
	$sales_data = $page_content->getElementsByTagName("sale");
	$sales_count = $sales_data->length;

	$names = array_fill(0, $sales_count, "");
	...
	$ids = array_fill(0, $sales_count, "");

	for ($i=0; $i < $sales_count; $i++) { 
		$names[$i] = $sales_data->item($i)->getElementsByTagName("name")->item(0)->textContent;
		$values[$i] = (int)$sales_data->item($i)->getElementsByTagName("value")->item(0)->textContent;

        @$descriptions[$i] = $sales_data->item($i)->getElementsByTagName("description")->item(0)->textContent;
        if (!$descriptions[$i]) {
            $descriptions[$i] = "";
        }
...
<div class="container-fluid content">
    ...
    <div class="col-fix">
        <div class="row hours-row">
            <span class="title">Количество часов: </span>

            <input type="radio" class="css-checkbox" id="radio1" checked="checked" name="hours" value="1" />
            <label for="radio1" class="css-label">1</label>

            <input type="radio" class="css-checkbox" id="radio2" name="hours" value="2" />
            <label for="radio2" class="css-label">2</label>

            <input type="radio" class="css-checkbox" id="radio3" name="hours" value="3" />
        ...
        <div class="row price-row">
            ...
            <div class="container-fluid button">
                <div class="row">
                    <div class="col-xs-9">
                        <span>
                            <span id="fix-price"><?php echo $fix_price; ?></span> р/час
                        </span>
                        <span>
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <span>
                            <span id="sum"><?php echo $fix_price; ?></span>р
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="popup-content">
            <form name="order-form" method="POST" action="" id="order-form">
                ...
                <div class="row">
                    ...
                    <div class="col-xs-3">
                        <label for="email-filed">E-mail</label>
                    </div>
                    <div class="col-xs-8">
...
<div class="popup" id="popup-info">
    <div class="popup-window info">
        ...
        <div class="popup-content">
            <div class="text">
                <span>
                    Указывается цена заказа из расчета на всех участников. Количество участников может варьироваться до 20 человек.
                    Одновременно в поле может присутствовать не более 10 человек (игры проводятся либо 3:3, либо 4:4, либо 5:5 человек).
                </span>
            </div>
        </div>
    </div>
